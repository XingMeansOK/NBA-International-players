...
function MapboxWrapper( map ) {
  ...
  this.synchronize(); // 同步three.js摄像机和mapboxgl摄像机，这个函数调用之后两个摄像机将一直保持同步（同步视图矩阵和投影矩阵）
                              // 同时同步场景中顶层节点 layercontainer的变换矩阵，使地图和three object3d的平移缩放保持一致
...
Object.assign( MapboxWrapper.prototype, {
  ...
  /*
    同步three.js的mvp矩阵和mapboxgl mvp矩阵，并保持同步
  */
  synchronize: function() {
    ...
    function _synchronize() {
      ...
      */
      ...
      // 重构transform._calcMatrices()方法，将mvp矩阵分离成mv和p矩阵
